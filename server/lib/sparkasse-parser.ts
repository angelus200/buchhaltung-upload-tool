...
export interface SparkasseHeader {
  format: 'SPARKASSE';
  columns: string[];
  hasValidHeader: boolean;
}

export interface SparkassePosition {
  datum: Date;
  buchungstext: string;
  betrag: number;
  referenz?: string;
...
export interface SparkasseParseResult {
  header: SparkasseHeader;
  positionen: SparkassePosition[];
  errors: string[];
  warnings: string[];
  stats: {
    totalRows: number;
...
/**
 * Prüft ob die CSV-Datei im Sparkasse-Format vorliegt
 */
export function isValidSparkasseFile(content: string): boolean {
  ...
  // Sparkasse CSV hat typischerweise diese Header-Spalten
  return firstLine.includes('Auftragskonto') &&
         firstLine.includes('Buchungstag') &&
         firstLine.includes(';'); // Semikolon als Delimiter
}

/**
 * Parst eine Sparkasse CSV-Datei und extrahiert alle Positionen
 * Format: Semikolon-getrennt, Werte in Anführungszeichen, deutsche Zahlenformate
 */
export function parseSparkasseCSV(csvContent: string): SparkasseParseResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  const positionen: SparkassePosition[] = [];
  ...
  if (lines.length === 0) {
    ...
    return {
      header: { format: 'SPARKASSE', columns: [], hasValidHeader: false },
      positionen: [],
      errors,
      warnings,
  ...
  if (!header.hasValidHeader) {
    errors.push('Ungültiger CSV-Header - Sparkasse-Format erwartet');
    return {
      header,
      positionen: [],
      errors,
  ...
  const beguenstigterIndex = columns.indexOf('Beguenstigter/Zahlungspflichtiger');
  const infoIndex = columns.indexOf('Info');
  ...
  if (verwendungszweckIndex === -1) {
    errors.push('Spalte "Verwendungszweck" nicht gefunden');
  }
  if (betragIndex === -1) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Buchungstext (Verwendungszweck ist Pflichtfeld)
    const buchungstext = fields[verwendungszweckIndex] || '';
    if (!buchungstext) {
      posErrors.push('Verwendungszweck fehlt');
    }

    // Optionale Felder
    const referenz = fields[beguenstigterIndex] && fields[beguenstigterIndex].length > 0
      ? fields[beguenstigterIndex]
      : undefined;

    const kategorie = fields[infoIndex] && fields[infoIndex].length > 0
      ? fields[infoIndex]
      : undefined;
